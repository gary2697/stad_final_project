...
/*heres  our whitebox testing for the calculate class
the class is mainly used to judgewin, add a point, calculate the next best possible move for the computer(pingjia methods)
 we aim for 100% branch coverage but there are unreachable code in all of the pingjia methods because it changes the board state after
the first iteration and make it impossible to have five stones in a row for any subsequent switch cases
we perform our whitebox testing mainly through testing all the directions and possibilties for each function
for instance, we test four possible directions that could result in a winning condition as well as both positive and negative directions
we also did boundary checks by inputing invalid states and coordinates
we also tested the program based on the color that the player uses, both black and white and the state of the board of his turn or his opponent's turn
*/
//failure reports:
/*
Pingjia3 missing one diag (x-k)(y-k) same with pingjia4,5,6
For pingjia 4 5 6, it only checks one direction
Starting pingjia 7, both directions are checked , but one branch is missing
In pingjia8 9 did  not check second half row/ check both directions of column checked one diagnol not the other
Pingjia10 miss half column miss one diagnal
Pingjia starts with 4 instead of 1 for both i and j Only enters the if statement if the board has been edited to neither 1 nor -1 Which causes us unable to reach 100% bc, switch case 5 can never be reached because of the statement aboveFirst time all branches both directions checked but since it starts with 4, it does not check the whole board.
Pingjia partially overlaps with pingjia i (where i = 2-9)
Jinshou variable does not affect the output
Fault for getNext(), the returned state of the point is not 1/0/-1, which makes it unable to test the state.
Use probe the range to test getNext since it calls the math.random function
Pingjia would not detect stones that are placed in two different subgrids (for instance, each pingjia checks a subgrid, if 3 stones on subgrid 1 and 2 stones on subgrid 2, it does not detect win)
Pingjia does not check the scenario where there is three stones in a row/col/diag and a black and another stone, which is also a winnning condition
3+1+1 / 2+1+2 etc.

 */
/*
some propose fixes to the calculate class is first, not to divide the pingjia method into 10 which is causing the problems of the program not being
able to detect stones on two seperate subgrids
pingjia method should not be changing the state of the board, should either make a copy of the board or reverse the changes after the evaluation
the judgewin method should also be changed so that every time the method is called, it checks the whole state of the board instead of a specific colmun
more rigorous boundry checking should be added
pingjia method should not simply return 0/1, which makes it extremely difficult to debug
 */
public class calculateTest {
    ...
    @Test // test add an out of bound  point exception
